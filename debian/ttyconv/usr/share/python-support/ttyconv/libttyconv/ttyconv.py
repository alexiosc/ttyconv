...
class TTYConv (object):
    ...
    def __init__(self):
        ...
        if not self.cmdline:
            self.cmdline = filter (None, [os.environ.get('SHELL')]) # pylint:disable-msg=W0141
        if not self.cmdline:
            self.cmdline = ['/bin/sh']
    ...
    def _lenientCodec (self, codec, string, replace='?'):
        """
    ...
    def lenientDecode (self, string, encoding, replace='?'):
        ...
        """
        return self._lenientCodec (lambda x: x.decode (encoding),
    ...
    def lenientEncode (self, string, encoding, replace='?'):
        ...
        '?' (or the value of ``replace``).
        """
        return self._lenientCodec (lambda x: x.encode (encoding),
                                   string, replace=replace)
